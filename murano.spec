%global full_release murano-%{version}
...
#cd %{_builddir}/%{full_release} && oslo-config-generator --config-file ./etc/oslo-config-generator/murano.conf
install -p -D -m 640 %{_builddir}/%{full_release}/etc/murano/murano.conf.sample %{buildroot}%{_sysconfdir}/murano/murano.conf
install -p -D -m 640 %{_builddir}/%{full_release}/etc/murano/netconfig.yaml.samle %{buildroot}%{_sysconfdir}/murano/netconfig.yaml.samle
install -p -D -m 640 %{_builddir}/%{full_release}/etc/murano/murano-paste.ini %{buildroot}%{_sysconfdir}/murano/murano-paste.ini
install -p -D -m 640 %{_builddir}/%{full_release}/etc/murano/policy.json %{buildroot}%{_sysconfdir}/murano/policy.json
install -p -D -m 640 %{_builddir}/%{full_release}/etc/murano/logging.conf.sample %{buildroot}%{_sysconfdir}/murano/logging.conf

# Copy 'meta' folder(murano meta packages written in muranoPL with execution plan main minimal logic)
cp -r %{_builddir}/%{full_release}/meta %{buildroot}/var/cache/murano
...
Requires:      python-psutil >= 1.1.1, python-psutil < 2.0.0
...
%changelog
* Wed Sep 23 2015 Igor Yozhikov <iyozhikov@mirantis.com>
- Build for MOS8.0
- Version update

* Mon Feb 2 2015 Igor Yozhikov <iyozhikov@mirantis.com>
- Implementation of LP BP separate-murano-package
- Murano package is now splitted to murano-common, murano-api and murano-engine

* Thu Sep 11 2014 Dmitry Teselkin <dteselkin@mirantis.com>
- Correct package name used in spec
- Correct config files names (paste.ini and murano.conf)

* Tue Sep 9 2014 Igor Yozhikov <iyozhikov@mirantis.com>
- updated for LP BP
...
* Mon Mar 31 2014 built by Igor Yozhikov <iyozhikov@mirantis.com>
- v0.5 patches revision and update

* Fri Mar 28 2014 built by Igor Yozhikov <iyozhikov@mirantis.com>
- v0.5 preparation

* Tue Dec 17 2013 built by Igor Yozhikov <iyozhikov@mirantis.com>
- build number - 5
